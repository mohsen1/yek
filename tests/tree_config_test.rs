...
fn test_tree_options_from_config_file() {
    ...
    assert!(output_str.contains("Directory structure:"), 
        "tree_header option not working from config file. Output: {}", output_str);
}
...
fn test_tree_only_from_config_file() {
    ...
    assert!(output_str.contains("Directory structure:"), 
        "tree_only option not working from config file. Output: {}", output_str);
    assert!(!output_str.contains("fn main() {}"), 
        "tree_only should not show file contents. Output: {}", output_str);
}