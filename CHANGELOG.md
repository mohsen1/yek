...
- Complete architectural overhaul introducing a modular processing pipeline with explicit discovery, filtering, and formatting stages.
- Implement Copilot-suggested enhancements that streamline processing context handling and CLI ergonomics.
- Extend configuration flexibility by adding `accept_from` support for tree and debug options.

### Bug Fixes

- Resolve configuration loading regressions for tree-related boolean flags and clap-config-file compatibility issues.
- Fix `--output-dir` and `--output-name` handling so both options work together reliably.
- Address type mismatches and error handling regressions uncovered during the pipeline refactor.

### Testing

- Add comprehensive pipeline coverage with new helpers for discovery, filtering, and configuration scenarios.
- Introduce unit tests for `ProcessingContext`, `RealFileSystem`, and stdin/config workflows to raise coverage.

### Miscellaneous Tasks

- Clean up lint warnings, formatting, and debug logging across the refactored pipeline.
- Update dependencies including `anyhow`, `serde`, `serde_json`, `toml`, and `tiktoken-rs`.
